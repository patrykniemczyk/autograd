...
def load_model(filepath: str) -> tuple:
    ...
    if len(params) != len(param_values):
        raise ValueError(f"Parameter count mismatch: expected {len(params)}, got {len(param_values)}")
...
def create_optimizer_from_model(model: MLP, optimizer_state: Optional[Dict] = None) -> AdamW:
    """
...
def export_model_info(model: MLP, filepath: str) -> None:
    ...
    with open(filepath, 'w') as f:
        ...
        for i in range(len(layer_sizes) - 1):
            ...
            f.write(f"  Parameter range: {param_idx} - {param_idx + layer_params - 1}\n")
...
def load_latest_checkpoint(checkpoint_dir: str = "checkpoints") -> tuple:
    ...
    latest_file = max(checkpoint_files, key=lambda f: 
