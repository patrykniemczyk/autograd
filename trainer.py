...
def mean_squared_error(
    predictions: List[Variable], targets: List[Variable]
) -> Variable:
    """
...
class Trainer:
    ...
    def evaluate(
        self, dataset, metrics: Optional[List[str]] = None
    ) -> Dict[str, float]:
        """
        ...
        for epoch in range(epochs + 1):
            ...
            if epoch > 0:
                try:
                    train_loss = self.train_epoch(
                        dataset, verbose=verbose, loss_fn=loss_fn
                    )
            ...
            if epoch % validate_every == 0 or epoch == epochs:
                try:
                    ...
                    current_eval_loss = eval_metrics["mse"]
                    if early_stopping_patience:
                        if (
                            current_eval_loss
                            < best_eval_loss - early_stopping_min_delta
                        ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train an MLP model for polynomial regression"
    )

    # Model configuration
    parser.add_argument(
        "--input-size", type=int, default=1, help="Number of input features"
    )
    parser.add_argument(
        "--hidden-sizes",
        type=int,
    ...
    parser.add_argument(
        "--output-size", type=int, default=1, help="Number of output features"
    )
    parser.add_argument(
        "--activation",
        type=str,
    ...
    parser.add_argument(
        "--validate-every", type=int, default=10, help="Validate every N epochs"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--weight-decay", type=float, default=0.01, help="Weight decay factor"
    )
    parser.add_argument(
        "--beta1", type=float, default=0.9, help="Beta1 for AdamW optimizer"
    )
    parser.add_argument(
        "--beta2", type=float, default=0.999, help="Beta2 for AdamW optimizer"
    )
    ...
    parser.add_argument(
        "--noise", type=float, default=0.1, help="Noise level for dataset"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--load-model", type=str, default=None, help="Path to load pretrained model"
    )
    parser.add_argument(
