...
class Variable:
    ...
    def backward(self, gradient: Optional[float] = None) -> None:
        ...
        if math.isnan(gradient) or math.isinf(gradient):
            raise ValueError(f"Initial gradient cannot be NaN or infinite, got {gradient}")
    ...
    def __pow__(self, other: Union["Variable", float]) -> "Variable":
        ...
        out = Variable(result_data, _prev=(self,)
                       if not isinstance(other, Variable) else (self, other))
        ...
        def _backward() -> None:
            if isinstance(other, Variable):
                ...
                if self.data > 0:  # Avoid log of negative/zero
                    ...
                    warnings.warn("Taking logarithm of negative base in gradient computation")
            else:
                # d/dx(x^c) = c * x^(c-1)
